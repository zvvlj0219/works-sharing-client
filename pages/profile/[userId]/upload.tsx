...
const UploadPortfolio = () => {
    ...
    const onChangeDescribeHandler = useCallback((e:React.ChangeEvent<HTMLTextAreaElement>) => {
        if(!e.target.value) return setDescribeText('')

        setDescribeText(e.target.value)
    }, [])

    const onChangeworknameHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadPortfolio = async () => {
        if(
            !selectedfile || !session?.user?.name
        ) return 
        ...
        const postImageResponse = await fetch(`${baseUrl}/upload/save_image`, postImageOptions)
        const { result_image } = await postImageResponse.json() as { result_image: UploadFile }
        ...
        const postPortfolioResponse = await fetch(`${baseUrl}/upload/save_portfolio`,{
            method: 'POST',
            headers: {
        ...
        const {result } = await postPortfolioResponse.json() as { result: Portfolio}
                    ...
                    <div>
                        ...
                        <div className='url container'>
                            ...
                            <input
                                type='text'
                                value={worknameField}
                                onChange={(event: React.ChangeEvent<HTMLInputElement>) => onChangeworknameHandler(event)}
                            />
                        </div>
                        <div className='url container'>
                            <p>ここにurl</p>
                            <input
                                type='text'
                                value={urlField}
                                onChange={(event: React.ChangeEvent<HTMLInputElement>) => onChangeURLHandler(event)}
                            />
                        </div>
                        <div>
                            <p>ここに概要</p>
